...
class Manager {
  public:
    ...
    void doFFT();
    void setFFTResultMagnitude();
    void setFFTResult();
    void setSampleRate(double sample_rate);
    double getSampleRate();
    std::array<float[FFTConstants::FFT_LENGTH], 4> getFFTResultMagnitude();
  ...
  private:
    ...
    std::array<float[FFTConstants::FFT_LENGTH], 4> fft_result_magnitude = {0.0f}; // 0: left, 1: right, 2: mid, 3: side
    std::array<float[FFTConstants::FFT_LENGTH], 4> fft_result = {0.0f};           // it's real fft result
